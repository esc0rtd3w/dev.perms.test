...
public class ShizukuService extends Service<ShizukuUserServiceManager, ShizukuClientManager, ShizukuConfigManager> {
    ...
    public static void main(String[] args) {
        // Embedded/server overrides (kept backwards compatible for upstream).
        // Supported properties/args:
        //   -Dshizuku.manager.package=<package> / --manager-package=<package>
        //   -Dshizuku.permission=<permission> / --permission=<permission>
        //   -Dshizuku.library.path=<path> / --library-path=<path>
        applyEmbeddedServerConfig(
                System.getProperty("shizuku.manager.package"),
                System.getProperty("shizuku.permission"),
                null);
        if (args != null) {
            for (String a : args) {
                if (a == null) continue;
                if (a.startsWith("--manager-package=")) {
                    applyEmbeddedServerConfig(a.substring("--manager-package=".length()), null, null);
                } else if (a.startsWith("--permission=")) {
                    applyEmbeddedServerConfig(null, a.substring("--permission=".length()), null);
                } else if (a.startsWith("--library-path=")) {
                    applyEmbeddedServerConfig(null, null, a.substring("--library-path=".length()));
                }
            }
        }
    ...
    private static void applyEmbeddedServerConfig(String managerPackage, String permission, String libraryPath) {
        if (managerPackage != null) {
            String v = managerPackage.trim();
            if (!v.isEmpty()) {
                ServerConstants.setManagerApplicationId(v);
            }
        }
        if (permission != null) {
            String v = permission.trim();
            if (!v.isEmpty()) {
                rikka.shizuku.common.ShizukuConfig.setPermission(v);
                ServerConstants.PERMISSION = rikka.shizuku.common.ShizukuConfig.getPermission();
            }
        }
        if (libraryPath != null) {
            String v = libraryPath.trim();
            if (!v.isEmpty()) {
                System.setProperty("shizuku.library.path", v);
            }
        }
    }
    ...
    public static ApplicationInfo getManagerApplicationInfo() {
        return PackageManagerApis.getApplicationInfoNoThrow(ServerConstants.getManagerApplicationId(), 0, 0);
    }
    ...
    public void attachApplication(IShizukuApplication application, Bundle args) {
        ...
        isManager = ServerConstants.getManagerApplicationId().equals(requestPackageName);
        ...
        if (!isManager) {
            ...
            try {
                PermissionManagerApis.grantRuntimePermission(ServerConstants.getManagerApplicationId(),
    ...
    public void showPermissionConfirmation(int requestCode, @NonNull ClientRecord clientRecord, int callingUid, int callingPid, int userId) {
        ...
        PackageInfo pi = PackageManagerApis.getPackageInfoNoThrow(ServerConstants.getManagerApplicationId(), 0, userId);
        ...
        Intent intent = new Intent(ServerConstants.getRequestPermissionAction())
                .setPackage(ServerConstants.getManagerApplicationId())
    ...
    private ParcelableListSlice<PackageInfo> getApplications(int userId) {
        ...
        for (int user : users) {
            for (PackageInfo pi : PackageManagerApis.getInstalledPackagesNoThrow(PackageManager.GET_META_DATA | PackageManager.GET_PERMISSIONS, user)) {
                if (Objects.equals(ServerConstants.getManagerApplicationId(), pi.packageName)) continue;
    ...
    static void sendBinderToManger(Binder binder, int userId) {
        sendBinderToUserApp(binder, ServerConstants.getManagerApplicationId(), userId);
    }
    ...
    static void sendBinderToUserApp(Binder binder, String packageName, int userId, boolean retry) {
        ...
        try {
            ...
            extra.putParcelable(ServerConstants.getManagerApplicationId() + ".intent.extra.BINDER", new BinderContainer(binder));
