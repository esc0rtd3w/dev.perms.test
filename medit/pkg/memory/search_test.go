...
func TestFindMaskedDataInSplittedMemory(t *testing.T) {
	memory := []byte{
		0x50, 0x6c, 0x61, 0x79, 0x65, 0x72, 0x20, 0x49, 0x44, 0x3a, 0x20, 0xAA, 0xBB, 0xCC,
		0x50, 0x6c, 0x61, 0x79, 0x65, 0x72, 0x20, 0x49, 0x44, 0x3a, 0x20, 0x11, 0x22, 0x33,
	}
	searchBytes := []byte{0x50, 0x6c, 0x61, 0x79, 0x65, 0x72, 0x20, 0x49, 0x44, 0x3a, 0x20, 0x00, 0x00, 0x00}
	compareMask := []byte{0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x00, 0x00, 0x00}
	actual := []int{}
	anchorStart, anchorBytes := longestFixedMaskRun(searchBytes, compareMask)
	findMaskedDataInSplittedMemory(&memory, searchBytes, compareMask, anchorStart, anchorBytes, len(searchBytes), 0x100, 0, &actual)
	expected := []int{0x100, 0x10e}
	if !reflect.DeepEqual(actual, expected) {
		t.Errorf("got addr slice: %v\nexpected addr slice: %v", actual, expected)
	}
}
