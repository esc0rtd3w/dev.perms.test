...
public final class AdbKey {
    ...
    // Bundle Conscrypt so exportKeyingMaterial() is available without non-SDK API access.
    ...
    public SSLContext getSslContext() throws Exception {
        // Match upstream Shizuku (TLSv1.3), but use bundled Conscrypt provider so
        // exportKeyingMaterial() is available without hidden API access.
        SSLContext sslContext;
        try {
            sslContext = SSLContext.getInstance("TLSv1.3", CONSCRYPT_PROVIDER);
        } catch (Throwable ignored) {
            sslContext = SSLContext.getInstance("TLS", CONSCRYPT_PROVIDER);
        }
